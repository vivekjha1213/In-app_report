...
<head>
  ...
  <title>ibu_qa_voda_pin_verifyReport   PROMOTIONS</title>
  <meta charset="utf-8">
...
<body>
<div class="container">
  <h2>REPORT(ibu_qa_voda_pin_verifyReport  conversion tracker)</h2>
  <p>Promotions Details of ibu_qa_voda_pin_verifyReport.</p>
   <div class="text-right">
      ...
      <div class="black_box">
<a class="gold_text" href="http://beyondhealth.info/INAPP_API_REPORT/ibu_qa_voda_pin_verifyReport?date=<?=date("Y-m-d")?>">Refresh</a>
</div>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From ibu_qa_voda_pin_verify where DATE(date) between '$today_date' and '$today_date' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `ibu_qa_voda_pin_verify`  where DATE(date) between '$today_date' and '$today_date'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM ibu_qa_voda_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$today_date' and '$today_date'";
...
<br>
<?php
$sql1="SELECT * From ibu_qa_voda_pin_verify where date(date) between '$today_date' and '$today_date' order by id desc";
//exit();
//exit();
