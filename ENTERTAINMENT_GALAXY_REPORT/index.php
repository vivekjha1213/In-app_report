...
<head>
  ...
  <title>ENTERTAINMENT_GALAXY_</title>  
  <meta charset="utf-8">
...
<div class="container">
  ...
  <p>Details of  ENTERTAINMENT_GALAXY</p>
   <div class="text-right"> 
      ...
      <div class="black_box">
<a class="gold_text" href="http://beyondhealth.info/INAPP_API_REPORT/ENTERTAINMENT_GALAXY_REPORT?date=<?=date("Y-m-d")?>">Refresh</a>
</div>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From ENTERTAINMENT_GALAXY_Pin_Verify where DATE(date) between '$today_date' and '$today_date' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `ENTERTAINMENT_GALAXY_Pin_Verify`  where DATE(date) between '$today_date' and '$today_date'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM ENTERTAINMENT_GALAXY_Pin_Verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$today_date' and '$today_date'";
...
<br>
<?php
$sql1="SELECT * From ENTERTAINMENT_GALAXY_Pin_Verify where date(date) between '$today_date' and '$today_date' order by id desc";
//exit();
//exit();
