...
<head>
  ...
  <title> game club nazara om omantel</title>  
  <meta charset="utf-8">
...
<body>
<div class="container">
  <h2>REPORT(game club nazara om omantel conversion tracker)</h2>
  <p>Promotions Details of game club nazara om omantel</p>
   <div class="text-right"> 
      ...
      <div class="black_box">
<a class="gold_text" href="http://beyondhealth.info/INAPP_API_REPORT/game_club_nazara_new_om_omantel?date=<?=date("Y-m-d")?>">Refresh</a>
</div>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From Nazara_om_omantel_pin_verify where DATE(date) between '$today_date' and '$today_date' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `Nazara_om_omantel_pin_verify`  where DATE(date) between '$today_date' and '$today_date'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM Nazara_om_omantel_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$today_date' and '$today_date'";
...
<br>
<?php
$sql1="SELECT * From Nazara_om_omantel_pin_verify where date(date) between '$today_date' and '$today_date' order by id desc";
//exit();
//exit();
