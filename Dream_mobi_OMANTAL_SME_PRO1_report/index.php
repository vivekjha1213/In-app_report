...
<head>
  ...
  <title>Dream_mobi_OMANTAL_SME_PRO1</title>
  <meta charset="utf-8">
...
<body>
<div class="container">
  <h2>REPORT(Dream_mobi_OMANTAL_SME_PRO1 conversion tracker)</h2>
  <p>Dream_mobi_OMANTAL_SME_PRO1</p>
   <div class="text-right">
      ...
      <div class="black_box">
<a class="gold_text" href="http://beyondhealth.info/INAPP_API_REPORT/Dream_mobi_OMANTAL_SME_PRO1_report?date=<?=date("Y-m-d")?>">Refresh</a>
</div>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From in_app_pin_verify where DATE(date) between '$today_date' and '$today_date' and pubName='DREAM_MOBI' and serviceName='SHEMAROO_OMANTEL_SME' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `in_app_pin_verify`  where DATE(date) between '$today_date' and '$today_date' and pubName='DREAM_MOBI' and serviceName='SHEMAROO_OMANTEL_SME'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM in_app_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$today_date' and '$today_date' and pubName='DREAM_MOBI' and serviceName='SHEMAROO_OMANTEL_SME'";
...
<br>
<?php
$sql1="SELECT * From in_app_pin_verify where date(date) between '$today_date' and '$today_date' and pubName='DREAM_MOBI' and serviceName='SHEMAROO_OMANTEL_SME' order by id desc";
//exit();
//exit();
