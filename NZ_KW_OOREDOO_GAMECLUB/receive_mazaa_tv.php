<head>
  <title>NZ_KW_OOREDOO_GAMECLUB AE Etisalat</title>
  <meta charset="utf-8">
...
<br>
<?php
$sql3="SELECT status,count(status) as counter From in_app_pin_verify where DATE(date) between '$from' and '$modified_to' and pubName='OLIMOB' and serviceName='NZ_KW_OOREDOO_GAMECLUB' group by status";
//exit();
...
<?php
    ...
    <!-- //CODE FOR CR -->
    <?php
    $sql="SELECT  count(distinct msisdn) as counter  FROM `in_app_pin_verify`  where DATE(date) between '$from' and '$modified_to' and pubName='OLIMOB' and serviceName='NZ_KW_OOREDOO_GAMECLUB'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM in_app_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$from' and '$modified_to' and pubName='OLIMOB' and serviceName='NZ_KW_OOREDOO_GAMECLUB'";
...
<br>
<?php
$sql1="SELECT * From in_app_pin_verify where date(date) between '$from' and '$modified_to' and pubName='OLIMOB' and serviceName='NZ_KW_OOREDOO_GAMECLUB' order by id desc";
//exit();
//exit();
