...
<head>
  ...
  <title>NZ_KW_OOREDOO_GAMECLUB </title>
  <meta charset="utf-8">
...
<body>
<div class="container">
  <h2>REPORT(NZ_KW_OOREDOO_GAMECLUB conversion tracker)</h2>
  <p>NZ_KW_OOREDOO_GAMECLUB</p>
   <div class="text-right">
      ...
      <div class="black_box">
<a class="gold_text" href="http://beyondhealth.info/INAPP_API_REPORT/NZ_KW_OOREDOO_GAMECLUB?date=<?=date("Y-m-d")?>">Refresh</a>
</div>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From in_app_pin_verify where DATE(date) between '$today_date' and '$today_date' and pubName='OLIMOB' and serviceName='NZ_KW_OOREDOO_GAMECLUB' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `in_app_pin_verify`  where DATE(date) between '$today_date' and '$today_date' and pubName='OLIMOB' and serviceName='NZ_KW_OOREDOO_GAMECLUB'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM in_app_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$today_date' and '$today_date' and pubName='OLIMOB' and serviceName='NZ_KW_OOREDOO_GAMECLUB'";
...
<br>
<?php
$sql1="SELECT * From in_app_pin_verify where date(date) between '$today_date' and '$today_date' and pubName='OLIMOB' and serviceName='NZ_KW_OOREDOO_GAMECLUB' order by id desc";
//exit();
//exit();
