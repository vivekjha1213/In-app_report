<head>
  <title> new bh uae PROMOTIONS</title>
  <meta charset="utf-8">
...
<br>
<?php
$sql3="SELECT status,count(status) as counter From new_beyondhealth_olimob_promotion where DATE(date) between '$from' and '$modified_to' group by status";
//exit();
...
<?php
    ...
    <!-- //CODE FOR CR -->
    <?php
    $sql="SELECT  count(distinct msisdn) as counter  FROM `new_beyondhealth_olimob_promotion`  where DATE(date) between '$from' and '$modified_to'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM new_beyondhealth_olimob_promotion  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$from' and '$modified_to'";
...
<br>
<?php
$sql1="SELECT * From new_beyondhealth_olimob_promotion where date(date) between '$from' and '$modified_to' order by id desc";
//exit();
//exit();
