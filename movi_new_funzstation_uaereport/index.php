...
<head>
  ...
  <title>moviplus_ex_new_funzstation_uae   PROMOTIONS</title>
  <meta charset="utf-8">
...
<body>
<div class="container">
  <h2>REPORT(moviplus_ex_new_funzstation_uae  conversion tracker)</h2>
  <p>Promotions Details of moviplus_ex_new_funzstation_uae.</p>
   <div class="text-right">
      ...
      <div class="black_box">
<a class="gold_text" href="http://beyondhealth.info/INAPP_API_REPORT/movi_new_funzstation_uaereport?date=<?=date("Y-m-d")?>">Refresh</a>
</div>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From movi_ex_new_funzstation_uae_alt_pin_verify where DATE(date_india) between '$today_date' and '$today_date' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `movi_ex_new_funzstation_uae_alt_pin_verify`  where DATE(date_india) between '$today_date' and '$today_date'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM movi_ex_new_funzstation_uae_alt_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date_india) between '$today_date' and '$today_date'";
...
<br>
<?php
$sql1="SELECT * From movi_ex_new_funzstation_uae_alt_pin_verify where date(date_india) between '$today_date' and '$today_date' order by id desc";
//exit();
//exit();
