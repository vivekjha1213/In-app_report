<head>
  <title>INTELLISENSE_qa_oredo PROMOTIONS</title>
  <meta charset="utf-8">
...
<br>
<?php
$sql3="SELECT status,count(status) as counter From INTELLISENSE_QA_Oredo__verify where DATE(date) between '$from' and '$modified_to' group by status";
//exit();
...
<?php
    ...
    <!-- //CODE FOR CR -->
    <?php
    $sql="SELECT  count(distinct msisdn) as counter  FROM `INTELLISENSE_QA_Oredo__verify`  where DATE(date) between '$from' and '$modified_to'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM INTELLISENSE_QA_Oredo__verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$from' and '$modified_to'";
...
<br>
<?php
$sql1="SELECT * From INTELLISENSE_QA_Oredo__verify where date(date) between '$from' and '$modified_to' order by id desc";
//exit();
//exit();
