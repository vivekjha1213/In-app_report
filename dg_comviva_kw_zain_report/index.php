...
<head>
  ...
  <title>INHOUSE dg_comviva_kw_zain_report   PROMOTIONS</title>
  <meta charset="utf-8">
...
<body>
<div class="container">
  <h2>REPORT(dg_comviva_kw_zain_report  conversion tracker)</h2>
  <p>Promotions Details of dg_comviva_kw_zain_report.</p>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From dg_comviva_kw_zain_pin_verify where DATE(date) between '$today_date' and '$today_date' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `dg_comviva_kw_zain_pin_verify`  where DATE(date) between '$today_date' and '$today_date'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM dg_comviva_kw_zain_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$today_date' and '$today_date'";
...
<br>
<?php
$sql1="SELECT * From dg_comviva_kw_zain_pin_verify where date(date) between '$today_date' and '$today_date' order by id desc";
//exit();
//exit();
