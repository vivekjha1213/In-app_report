...
<head>
  ...
  <title>direct_om_oredo_report_gamecafe  PROMOTIONS</title>
  <meta charset="utf-8">
...
<body>
<div class="container">
  <h2>REPORT(direct_om_oredo_report_gamecafe  conversion tracker)</h2>
  <p>Promotions Details of direct direct_om_oredo_report_gamecafe.</p>
   <div class="text-right">
      ...
      <div class="black_box">
<a class="gold_text" href="http://beyondhealth.info/INAPP_API_REPORT/direct_om_oredo_report_gamecafe?date=<?=date("Y-m-d")?>">Refresh</a>
</div>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From direct_game_mcomviva_ooredoo_pin_verify where DATE(date) between '$today_date' and '$today_date' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `direct_game_mcomviva_ooredoo_pin_verify`  where DATE(date) between '$today_date' and '$today_date'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM direct_game_mcomviva_ooredoo_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$today_date' and '$today_date'";
...
<br>
<?php
$sql1="SELECT * From direct_game_mcomviva_ooredoo_pin_verify where date(date) between '$today_date' and '$today_date' order by id desc";
//exit();
//exit();
