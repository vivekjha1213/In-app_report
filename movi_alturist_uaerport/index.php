...
<head>
  ...
  <title>movi_alturist_uaereport   PROMOTIONS</title>
  <meta charset="utf-8">
...
<body>
<div class="container">
  <h2>REPORT(movi_alturist_uaereport  conversion tracker)</h2>
  <p>Promotions Details of movi_alturist_uaereport.</p>
   <div class="text-right">
      ...
      <div class="black_box">
<a class="gold_text" href="http://beyondhealth.info/INAPP_API_REPORT/movi_alturist_uaerport?date=<?=date("Y-m-d")?>">Refresh</a>
</div>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From movi_ex_new_game2play_uae_etisalat_pin_verify where DATE(date) between '$today_date' and '$today_date' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `movi_ex_new_game2play_uae_etisalat_pin_verify`  where DATE(date) between '$today_date' and '$today_date'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM movi_ex_new_game2play_uae_etisalat_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$today_date' and '$today_date'";
...
<br>
<?php
$sql1="SELECT * From movi_ex_new_game2play_uae_etisalat_pin_verify where date(date) between '$today_date' and '$today_date' order by id desc";
//exit();
//exit();
