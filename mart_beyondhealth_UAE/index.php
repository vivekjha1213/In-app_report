...
<head>
  ...
  <title>MOBILEART_beyondhealth_UAE   PROMOTIONS</title>
  <meta charset="utf-8">
...
<body>
<div class="container">
  <h2>REPORT(MOBILEART_beyondhealth_UAE  conversion tracker)</h2>
  <p>Promotions Details of MOBILEART_beyondhealth_UAE.</p>
   <div class="text-right">
      ...
      <div class="black_box">
<a class="gold_text" href="http://beyondhealth.info/INAPP_API_REPORT/mart_beyondhealth_UAE?date=<?=date("Y-m-d")?>">Refresh</a>
</div>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From mart_beyondhealth_olimob_promotion where DATE(date) between '$today_date' and '$today_date' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `mart_beyondhealth_olimob_promotion`  where DATE(date) between '$today_date' and '$today_date'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM mart_beyondhealth_olimob_promotion  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$today_date' and '$today_date'";
...
<br>
<?php
$sql1="SELECT * From mart_beyondhealth_olimob_promotion where date(date) between '$today_date' and '$today_date' order by id desc";
//exit();
//exit();
