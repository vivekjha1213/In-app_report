<head>
  <title>SHEMAROO_OMANTEL_ATLASReport   PROMOTIONS</title>
  <meta charset="utf-8">
...
<br>
<?php
$sql3="SELECT status,count(status) as counter From atlas_she_omantel_verify where DATE(date) between '$from' and '$modified_to' and cid='ATLAS' group by status";
//exit();
...
<?php
    ...
    <!-- //CODE FOR CR -->
    <?php
    $sql="SELECT  count(distinct msisdn) as counter  FROM `atlas_she_omantel_verify`  where DATE(date) between '$from' and '$modified_to' and cid='ATLAS'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM atlas_she_omantel_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$from' and '$modified_to' and cid='ATLAS'";
...
<br>
<?php
$sql1="SELECT * From atlas_she_omantel_verify where date(date) between '$from' and '$modified_to' and cid='ATLAS' order by id desc";
//exit();
//exit();
