<head>
  <title>BEYOND_HEALTH_UAE_BHAVYESHReport  PROMOTIONS</title>
  <meta charset="utf-8">
...
<br>
<?php
$sql3="SELECT response,count(response) as counter From bhavyesh_tpay_uae_pin_verify where DATE(date) between '$from' and '$modified_to' group by response";
//exit();
...
<?php
    ...
    <!-- //CODE FOR CR -->
    <?php
    $sql="SELECT  count(distinct msisdn) as counter  FROM `bhavyesh_tpay_uae_pin_verify`  where DATE(date) between '$from' and '$modified_to'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM bhavyesh_tpay_uae_pin_verify  where `response` IN ('PASSED', 'BLOCK') AND DATE(date) between '$from' and '$modified_to'";
...
<br>
<?php
$sql1="SELECT * From bhavyesh_tpay_uae_pin_verify where date(date) between '$from' and '$modified_to' order by id desc";
//exit();
//exit();
