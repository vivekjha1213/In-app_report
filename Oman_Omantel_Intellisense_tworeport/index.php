...
<head>
  ...
  <title>Oman_Omantel_Intellisense_tworeport   PROMOTIONS</title>
  <meta charset="utf-8">
...
<body>
<div class="container">
  <h2>REPORT(Oman_Omantel_Intellisense_tworeport  conversion tracker)</h2>
  <p>Promotions Details of Oman_Omantel_Intellisense_tworeport.</p>
   <div class="text-right">
      ...
      <div class="black_box">
<a class="gold_text" href="http://beyondhealth.info/INAPP_API_REPORT/Oman_Omantel_Intellisense_tworeport?date=<?=date("Y-m-d")?>">Refresh</a>
</div>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From two_Oman_Omantel_Intellisense_pin_verify where DATE(date) between '$today_date' and '$today_date' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `two_Oman_Omantel_Intellisense_pin_verify`  where DATE(date) between '$today_date' and '$today_date'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM two_Oman_Omantel_Intellisense_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$today_date' and '$today_date'";
...
<br>
<?php
$sql1="SELECT * From two_Oman_Omantel_Intellisense_pin_verify where date(date) between '$today_date' and '$today_date' order by id desc";
//exit();
//exit();
