<head>
  <title>JW_PLS_TWOReport   PROMOTIONS</title>
  <meta charset="utf-8">
...
<br>
<?php
$sql3="SELECT status,count(status) as counter From jw_pl_pin_verify where DATE(date) between '$from' and '$modified_to' group by status";
//exit();
...
<?php
    ...
    <!-- //CODE FOR CR -->
    <?php
    $sql="SELECT  count(distinct msisdn) as counter  FROM `jw_pl_pin_verify`  where DATE(date) between '$from' and '$modified_to'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM jw_pl_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$from' and '$modified_to'";
...
<br>
<?php
$sql1="SELECT * From jw_pl_pin_verify where date(date) between '$from' and '$modified_to' order by id desc";
//exit();
//exit();
