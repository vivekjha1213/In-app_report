<head>
  <title>FUNZSTATION_UAE_IGKReport   PROMOTIONS</title>
  <meta charset="utf-8">
...
<br>
<?php
$sql3="SELECT status,count(status) as counter From gamespad_promotion where DATE(date) between '$from' and '$modified_to'  and cid='igknight' group by status";
//exit();
...
<?php
    ...
    <!-- //CODE FOR CR -->
    <?php
    $sql="SELECT  count(distinct msisdn) as counter  FROM `gamespad_promotion`  where DATE(date) between '$from' and '$modified_to' and cid='igknight'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM gamespad_promotion  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$from' and '$modified_to' and cid='igknight'";
...
<br>
<?php
$sql1="SELECT * From gamespad_promotion where date(date) between '$from' and '$modified_to' and cid='igknight' order by id desc";
//exit();
//exit();
