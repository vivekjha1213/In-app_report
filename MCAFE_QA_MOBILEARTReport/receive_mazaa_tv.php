<head>
  <title>MCAFE_QA_MOBILEARTReport   PROMOTIONS</title>
  <meta charset="utf-8">
...
<br>
<?php
$sql3="SELECT status,count(status) as counter From qatar_mobimind_pin_verify where DATE(Date) between '$from' and '$modified_to' and cid='Mobileart' group by status";
//exit();
...
<?php
    ...
    <!-- //CODE FOR CR -->
    <?php
    $sql="SELECT  count(distinct msisdn) as counter  FROM `qatar_mobimind_pin_verify`  where DATE(Date) between '$from' and '$modified_to' and cid='Mobileart'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM qatar_mobimind_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(Date) between '$from' and '$modified_to' and cid='Mobileart'";
...
<br>
<?php
$sql1="SELECT * From qatar_mobimind_pin_verify where DATE(Date) between '$from' and '$modified_to' and cid='Mobileart' order by id desc";
//exit();
//exit();
...
<table class="table table-striped">
  ...
  <th>DATE</th>
    ...
    <td><?php echo $row3['status_details']; ?></td>
