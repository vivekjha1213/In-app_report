...
<head>
  ...
  <title>PURETECH_OM_OMANTELreport   PROMOTIONS</title>
  <meta charset="utf-8">
...
<body>
<div class="container">
  <h2>REPORT(PURETECH_OM_OMANTELreport  conversion tracker)</h2>
  <p>Promotions Details of PURETECH_OM_OMANTELreport.</p>
   <div class="text-right">
      ...
      <div class="black_box">
<a class="gold_text" href="http://beyondhealth.info/INAPP_API_REPORT/PURETECH_OM_OMANTELreport?date=<?=date("Y-m-d")?>">Refresh</a>
</div>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From puretech_om_omantel_pin_verify where DATE(date) between '$today_date' and '$today_date' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `puretech_om_omantel_pin_verify`  where DATE(date) between '$today_date' and '$today_date'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM puretech_om_omantel_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$today_date' and '$today_date'";
...
<br>
<?php
$sql1="SELECT * From puretech_om_omantel_pin_verify where date(date) between '$today_date' and '$today_date' order by id desc";
//exit();
//exit();
